...
define([
	...
	*/
	...
	//====== Convert game to screen location
	me.convertWorldToScreen = function(location, relPos){
		var adjusted_x = (location.x - relPos.x) + canvas.width/2;
        var adjusted_y = (location.y - relPos.y) + canvas.width/2;

        var result = new Vector2(adjusted_x, adjusted_y);
        return result;
	};
	me.convertScreenToWorld = function(location, relPos){

		var world = location.mimic();
		world.adjustX(-canvas.width/2);
		world.adjustY(-canvas.width/2);

		world.adjustX(relPos.x);
		world.adjustY(relPos.y);
